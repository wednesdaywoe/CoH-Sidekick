...
**Two shas that differ do not say why.** Moved-forward and rewritten-away printed one sentence
between them, and the remedy for the first erases the evidence of the second. Separate voices now.

**A check that reports without stopping answers the question it just refused.** The new shape
guard rejected a head that is not a sha and fell through anyway, so a head reading `HEAD` came
back as ordinary drift — `HEAD` resolves in every repo. Caught by taking the branch, not reading it.
...
server settled it in one answer: a piece takes a booster exactly when it enhances something, which
is what the rule already read. What was missing was anyone asking the game.
...
was the reader's outright. The round trip measures the pair, never the hop.

The last of them closed by deciding rather than measuring: three code paths wrote a slot level
...
took its pieces out of both tallies (MBDIMPORT-5). Both closed 2026-09-09.
...
Its names dump was re-cut the same day, against the `I12.mhd` it had always come from — same sha,
every row identical, only the powerset keys' case restored. Thunderspy went from no path table at
all to 3,453 paths, and from 386 unnamed powersets to the 14 the census already attributed.

**The last to close was MBDEXPORT-10**, surfaced by that closure's regen. An unlettered member
went to the last free slot, and Rebirth is the one database with two — so one set's rez proc bound
to our "Recharge" piece, both directions. Mids' member order places it now.
...
[REBUILD-PROGRESS](REBUILD-PROGRESS.md)'s `.mbd` row is the live work. It carries the
reconciliation MBDIMPORT-5 landed, so the Rust reader asserts it rather than inheriting it.
...
- [x] **BOOST-7** — 128 stored `+5`s over 53 set pieces in a 2,178-file corpus, stripped
  and reported, never graded against the game. Not one flag doing four jobs: 95
  are aspect-less procs, 28 attuned Superior ATOs with no craft level to combine into, 5 ordinary
  pieces slotted at 30. The live server named the discriminator — a level-50 Gladiator's Armor
  Res/End/Rech boosts, the same set's aspect-less `#6` and Zephyr's static 4 points do not — so a
  piece takes one exactly when it enhances something, which the rule already read
...
- [x] **MBDIMPORT-5** — a power the reassign guard refused took its slots and its enhancements out of the build with it and neither tally counted them: the 3.7.5.21 Stalker file held 92 enhancements, imported 86 and reported `enhancementsFailed: 0`, so six pieces were in the file and in no number the user was shown; every declined entry is now reconciled against what it arrived holding, once at the foot of the loop rather than at each of a dozen refusal sites, and `slotsSkipped` makes `slotsImported` state what it counts
...
- [x] **MBDEXPORT-10** — Mids' Rebirth EnhDB letters `Return_From_the_Grave`'s sixth piece `_E`, a
  duplicate of its fifth, so the emitter had two free slots and one unlettered member
  (`_Rez_Effects`) and filed it LAST, binding the set's rez proc to our piece 6, "Recharge" —
...
- [x] **MBDEXPORT-21** — three code paths wrote a level into `slotOrder` and nothing recorded
  ...
  states `levelSource`, an authored level rides out and a packed one is re-solved and disclosed
  once, and `mids-export.ts` no longer forks on slot levels at all
...
- [x] **FORK-9** — one sentence for both ways a recorded head stops matching: the repo moved
  ...
  history, where it can only be re-taken. The beta's stamp named `623d9129ab`, which GitHub had
  and the repo did not — rebased away, and main is linear on purpose, so any stamp can be
  orphaned. Closed 2026-09-22: separate voices, a head that is not a sha refused as shape,
  `fork9-head-cases.test.ts` taking all five branches.
